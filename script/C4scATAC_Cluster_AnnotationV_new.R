...
counts <- FeatureMatrix(fragments = frag,features = grfile, cells=cells$V1)

## write peak-cell matrix to Matrix Market Formats
writeMM(counts, file=paste0(args$out,"/out/Peak/matrix.mtx"))
write.table(colnames(counts),file=paste0(args$out,"/out/Peak/barcodes.tsv"),quote=F,row.names=F,col.names=F)
write.table(rownames(counts),file=paste0(args$out,"/out/Peak/peak.bed"),quote=F,row.names=F,col.names=F)
...
ggsave(paste0(args$out,"/report/plot4_QC.png"), do.call(plot_grid,c(QC_plot, ncol = 3, align = "hv")), width = 7, height = 4)
ggsave(paste0(args$out,"/report/plot4_QC.svg"), do.call(plot_grid,c(QC_plot, ncol = 3, align = "hv")), width = 7, height = 4)
...
write.table(qc5,paste0(args$out,"/report/5.library.QC.csv"),sep = ":",quote = FALSE,row.names = FALSE,col.names = FALSE)
...
write.table(qc1,paste0(args$out,"/report/1.cell_report.csv"),sep = ":",quote = FALSE,row.names = FALSE,col.names = FALSE)
...
png(paste0(args$out,"/report/plot3_DropBeadsnum.png"),width = 458,height = 377)
...
svg(paste0(args$out,"/report/plot3_DropBeadsnum.svg"),width = 5,height = 4)
print(p)
dev.off()
...
png(paste0(args$out,"/report/plot7_Cluster_peak.png"),width = 524,height = 488)
print(c1)
dev.off()

svg(paste0(args$out,"/report/plot7_Cluster_peak.svg"),width = 5,height = 5)
print(c1)
dev.off()
...
png(paste0(args$out,"/report/plot8_Cluster_depth.png"),width = 524,height = 488)
print(c2)
dev.off()

svg(paste0(args$out,"/report/plot8_Cluster_depth.svg"),width = 6,height = 5)
print(c2)
dev.off()
...
write.table(m1,paste0(args$out,"/report/3.mapping.csv"),sep = ":",quote = FALSE,row.names = FALSE,col.names = FALSE)
rm(MT)
rm(m1)
...
promoter <- FeatureMatrix(fragments = frag,features = grpromoter, cells=cells$V1)
writeMM(promoter, file=paste0(args$out,"/out/Promoter/matrix.mtx"))
write.table(colnames(promoter),file=paste0(args$out,"/out/Promoter/barcodes.tsv"),quote=F,row.names=F,col.names=F)
write.table(rownames(promoter),file=paste0(args$out,"/out/Promoter/promoter.bed"),quote=F,row.names=F,col.names=F)
rm(tmp)
...
if (length(scATAC_result$scMCA) != 0){
  ...
  png(paste0(args$out,"/report/plot9_Cluster_annotation.png"),width = 990,height = 479)
  print(c3)
  dev.off()

  svg(paste0(args$out,"/report/plot9_Cluster_annotation.svg"),width = 11,height = 5)
  print(c3)
  dev.off()
  ...
  write.table(order_table_cell.type,paste0(args$out,"/report/Table2.celltypecount.csv"),sep = ",",quote = FALSE,row.names = FALSE)

} else{
  table_cell.type=data.frame(predicated.cell.type="None",number=dim(scATAC)[2],ratio=1)
  write.table(table_cell.type,paste0(args$out,"/report/Table2.celltypecount.csv"),sep = ",",quote = FALSE,row.names = FALSE)
}
saveRDS(scATAC,paste0(args$out,"/report/saved_clustering.rds"))
...
sub_da_peak$chr=unlist(lapply(strsplit(sub_da_peak$gene,"-"),"[",1))
sub_da_peak$start=unlist(lapply(strsplit(sub_da_peak$gene,"-"),"[",2))
sub_da_peak$end=unlist(lapply(strsplit(sub_da_peak$gene,"-"),"[",3))
...
names(out_TSS_GRange) <- make.unique(names(out_TSS_GRange))
...
write.table(result_da_peak,paste0(args$out,"/report/Table1.dapeak.csv"),sep = ",",quote = FALSE,row.names = FALSE)
