...
writeMM(counts, file=paste0(args$out,"/out/matrix.mtx"))
...
my_meta$FRIP=colSums(counts)/2/my_meta$uniqueNuclearFrags
...
QC_plot[[1]] <- ggplot(my_meta, aes(x = Sample, y = log10_uniqueFrags, fill=Sample)) + 
    ...
    geom_boxplot(width=0.06,position=position_dodge(0.9),show.legend = F,fill="white")+ 
...
ggsave(paste0(args$out,"/report/plot4_QC.png"), do.call(plot_grid,c(QC_plot, ncol = 3, align = "hv")), width = 523, height = 428)
